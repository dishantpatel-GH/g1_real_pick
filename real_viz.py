#!/usr/bin/env python3
# =============================================================================
# real_viz.py — tune/visualize the grasp on the REAL detected glass (cuRobo, no robot). Same methods +
# tuned sliders as sim_grasp_viz, but the glass is loaded from /tmp/glass_pose.json (object_detection --emit).
#   SideGrasp (yaw 0) | FrontGrasp (FG_YAW) | BestAngle (most-front reachable) | Coverage sweep.
# Tune SG_DX/SG_DYGAP/WRIST X-Y nudge/FROMTOP/APPROACH/FG_YAW/PITCH, then bake the values into
# real_plan_approach.py (which uses the same BestAngle yaw sweep).
#
# RUN (cumotion_venv): /home/dishant/cumotion_venv/bin/python real_viz.py
# =============================================================================
import sys, time, threading, traceback, json, os
import numpy as np

OUT  = "/home/dishant/g1_ws/cumotion/config/g1_inspire_right.yml"
PORT = 8080
# Glass loaded from the REAL detection (object_detection --emit) if present, else a default. Same grasp
# methods/params as sim_grasp_viz (SideGrasp / FrontGrasp / BestAngle + the tuned sliders).
_POSE = "/tmp/glass_pose.json"
if os.path.exists(_POSE):
    _P = json.load(open(_POSE)); CX, CY = float(_P["center"][0]), float(_P["center"][1])
    R = float(_P["radius_m"]); H = float(_P["height_m"]); CZ = float(_P["top_z"]) - H / 2.0
    print(f"[viz] REAL glass from {_POSE}: center=({CX:+.3f},{CY:+.3f}) r={R:.3f} h={H:.3f}")
else:
    R, H = 0.036, 0.105; CX, CY = 0.42, -0.24; CZ = -0.006 + H / 2.0
    print("[viz] no /tmp/glass_pose.json — using default glass")
